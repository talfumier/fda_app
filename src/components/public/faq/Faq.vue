<script setup>
  ...
  onMounted(async () => {  
    ...
    try {
      ...
      state.value= Object.values(groupsById).sort(
        (a, b) => a.order - b.order
      )
    } catch (error) {
...
<template>
  <div class="page">
    <section v-for="cat in state"
      :key="cat.idType">
      <header :class="[cat.expand?'border':'']"
        @click="cat.expand=!cat.expand"
      >        
      ...
      <article v-for="(faq,idx) in cat.items"
        v-if="cat.expand"
        :key="faq.idFaq"
      >
        <ul>
          <li class='question'>
            <q-icon name="keyboard_arrow_down" size="3rem" 
              :style="`transform: rotate(${faq.expand?'-180deg':'0deg'});transition: 0.6s ease;`"
              @click="faq.expand=!faq.expand"
            >
            </q-icon>
            {{ faq[`question_${locale}`]}}</li>
          <li :class="['answer',faq.expand?'visible':'hidden']">
            {{faq[`answer_${locale}`]}}
          </li>
        </ul>
        <FileViewer v-if="faq.idFile && faq.expand"
