...
import { toastError } from '@/components/common/toast/toast.js'
...
axios.interceptors.response.use(
  ...
  async (error) => {
    ...
    let text = 'Oups ... an unexpected error has occured >>> '    
    if (!error.response)  //no response means network error
      text=text + 'network or connection not available !'
    else {
